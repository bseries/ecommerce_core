...
class Products extends \base_core\models\Base {
	...
	public static function syncWithRemoteStock() {
		foreach (static::all() as $product) {
			$r = $product->save([
				'stock' => $product->stock_remote
			], ['whitelist' => ['stock']]);

			if (!$r) {
				throw new Exception('Failed to sync with remote stock.');
			}
		}
		Logger::write('debug', 'Synchronized stock');
		return true;
	}
}
